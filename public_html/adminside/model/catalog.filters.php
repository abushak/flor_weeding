...
class CatalogFilters extends Model {
    public function addFilter($data, $files = null) {
        $_data_status = ($data['status'] == "on") ? 1 : 0;
        $this->db->query("INSERT INTO `zet_category_filters` (`filter_id`, `title`, `priority`) VALUES  ('', '', '".(int)$data['parent_category']."')");
        $filter_id = $this->db->getLastId();
        
        $this->_linkTransliteration($category_id, $data['url'], $data['title'][1], 'zet_category_filters');
        ...
        foreach ($data['title'] as $language_id => $value) {
            $this->db->query("INSERT INTO `zet_category_filters` (`filter_id`, `language_id`, `title`, `priority`) VALUES ('".(int)$filter_id."', '".(int)$language_id."', '".$this->db->escape($data['title'][$language_id])."', '".(int)$data['priority']."')");
        }
        
        $this->db->query("INSERT INTO `zet_category_filter` (`filter_id`, `filter_category_id`, `status`) VALUES ('".(int)$filter_id."', '".(int)$filter_id."'), '1'");
    }
    
    public function editFilter($filter_id, $data, $files = null) {
        $_data_status = ($data['status'] == "on") ? 1 : 0;
        $this->db->query("UPDATE `zet_category_filters` SET `parent_category`='".(int)$data['parent_category']."', `status`= '".$_data_status."' WHERE `id`='".(int)$category_id."'");
    ...
    public function getTotalFilters() {
        $query = $this->db->query("SELECT COUNT(*) AS `total` FROM `zet_category_filter`");
        $res = $this->db->assoc($query);
        return $res['total'];
    }
    ...
    public function BackendGetFiltersByStatus($parent = 0, $static = 0){
        global $lang_id; $data = array(); $subdata = array();
        //$data['item'] = array();
        $query = $this->db->query("SELECT `title`, `filter_category_id`,`priority`, `status` FROM `zet_category_filters`,`zet_category_filter` WHERE `zet_category_filter`.`filter_id`=`filter_category_id` AND `language_id` = '".$lang_id."' ORDER BY `priority` ASC");
        while($row = $this->db->assoc($query)){
