<section class="brand sub row sub_row" id="id_<?php echo $category_row["id"]; ?>">
    <section class="span7">
    ...
    <section class="center_text span2">
        ...
        <a href="<?php echo ADM_SRV."catalog/categories/".$category_row["id"]."/?act=delete" ?>" class="admin_order_delete" title="Видалити"></a>
    </section>
    
    <section class="sub_sub_rows_container">
    <aside></aside>
    <?php $sub_sub_query = $catalog_category->getCategories($site_language["id"], $category_row["id"]);
          while($category_row = $mysql->assoc($sub_sub_query)) {
              include ADM_TPL."catalog/categories/level_3.row.tpl";
    } ?>
    </section>
</section>
